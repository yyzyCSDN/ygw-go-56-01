...
// Rollback restores only the tables written by committed segments. Tables
// written by the in-flight segment are left in their imported state.
func (im *Importer) Rollback() error {
	if im.baseline == nil {
		return errors.New("ingest: no baseline captured")
	}
	tables := committedTables(im.segments)
