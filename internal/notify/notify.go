...
// Push delivers an event to every subscriber. A failed delivery is retried
// once, and the first persistent failure is returned to the caller so the
// mutation layer can surface it instead of silently dropping the event.
func (n *Notifier) Push(ctx context.Context, event model.ChangeEvent) error {
	...
	for _, sub := range subs {
		...
		if err != nil {
			attempts = 2
			retryErr := sub.Deliver(ctx, event)
			if retryErr != nil {
				n.record(event.Key(), attempts, retryErr)
				if firstErr == nil {
					firstErr = retryErr
				}
				continue
		...
		n.record(event.Key(), attempts, nil)
	}
	if firstErr != nil {
		return firstErr
	}
