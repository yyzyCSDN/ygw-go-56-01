...
func (m *Meta) Publish(tableID string) (*model.Version, error) {
	...
	m.mu.Unlock()
	if err := m.notifier.Push(context.Background(), event); err != nil {
		return nil, err
	}
	return version.Clone(), nil
}
...
// SwitchTo activates a specific version for a table. The requested version is
// not validated against the recorded versions before the switch.
